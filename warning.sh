...
config_warn_files=($common1 $common2 $common3 $common4 $common5)




#预警需要测试的数据 模型存放路径
path5='data/warning/test2/train.tsv'
path6='data/warning/test2/dev.tsv'
...
save_model_warn_paths=($s1 $s2 $s3 $s4 $s5)
...
s4=$save_model_warn_path$s4$s
s5=$save_model_senti_path$s5$s
save_model_warn_path_ditills=($s1 $s2 $s3 $s4 $s5)
...
train1 $path5 $path6 config_warn_files save_model_warn_paths save_model_warn_path_ditills 12 12
...
train2 $path5 $path6 config_warn_files save_model_warn_paths save_model_warn_path_ditills 35 30
COMMENT
