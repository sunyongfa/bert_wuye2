...
class FastBERTClassifier(nn.Module):
    ...
    def forward(self, hidden_states,attention_mask):
        ...
        states_output = self.selfAttention(states_output, attention_mask, use_attention_mask=True)
        ...
        # ------训练阶段, 第一阶段初始训练, 第二阶段蒸馏训练--------#
            ...
            if training_stage == 0:
                for layer_module in self.layers:
                    hidden_states = layer_module(hidden_states, attention_mask)
                logits = self.layer_classifier(hidden_states,attention_mask)
            ...
            # 蒸馏训练，每层的student和teacher的KL散度作为loss
                ...
                for encoder_layer, (k, layer_classifier_module) in zip(all_encoder_layers,
                    ...
                    layer_logits = layer_classifier_module(encoder_layer,attention_mask)
