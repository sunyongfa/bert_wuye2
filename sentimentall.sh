...
#情绪全部数据训练 模型存放路径
path3='data/sentiment/all/train.tsv'
path4='data/sentiment/all/dev.tsv'
...
save_model_allsenti_paths=($s1 $s2 $s3 $s4 $s5)
...
s4=$save_model_senti_path$s4$s
s5=$save_model_senti_path$s5$s
save_model_allsenti_path_distills=($s1 $s2 $s3 $s4 $s5)
...
train1 $path3 $path4 config_senti_files save_model_allsenti_paths save_model_allsenti_path_distills 14 14
...
train2 $path3 $path4 config_senti_files save_model_senti_paths save_model_senti_path_ditills 10 10
